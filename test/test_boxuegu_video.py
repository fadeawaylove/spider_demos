...
class BoXueGuSpider(object):
    ...
    def run(self):
        self.decode_ts_file()
        # resp = requests.get("https://p.bokecc.com/servlet/hlskey?info=030F2A399BAAA55C9C33DC5901307461&t=1572071585&key=30195EBD771C9006516E44A4952BF929", headers=self.m3u8_headers)
        # print(resp.content)


    def parse_m3u8(self):
        """解析m3ub文件得到ts文件的url地址"""
        resp = requests.get(self.m3u8_url, params=self.m3u8_params, headers=self.m3u8_headers)
        resp_data = resp.text
